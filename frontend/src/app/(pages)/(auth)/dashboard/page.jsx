...
function HamburgerMenu({ activePage, setPage }) {
  ...
  return (
    <>
      ...
      <div className="fixed left-0 right-0 z-20 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between lg:hidden"
        style={{ top: 'var(--navbar-height, 64px)' }}>
        {/* Brand */}
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: gradCard }}>
      ...
      `}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold" style={{ background: grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              MUFT MADAD
            </h1>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          <nav className="space-y-1">
            {nav.map((item, idx) =>
              item.divider ? (
                <p key={idx} className="text-xs font-bold text-gray-400 uppercase tracking-widest px-2 pt-4 pb-1">
                  {item.label}
                </p>
              ) : (
                <button
