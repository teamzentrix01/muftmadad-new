...
export default function TreatmentAdminForm() {
  ...
  const initialForm = useRef(formData);
  ...
  const handleSubmit = async (e, createNext = false) => {
    ...
    if (!formData.name.trim() || !formData.slug.trim() || !formData.comes_in.trim()) {
      setMessage({ type: 'error', text: 'Treatment name, slug and category (Comes In) are required.' }); setCurrentStep(1); return;
    }
    setIsSubmitting(true);
    setMessage({ type: "", text: "" });
    try {
      const record = await saveAdminRecord('/admin/create', '/admin', savedId, { ...formData, specialty_id: formData.specialty_id || null });
      setSavedId(record.id);
      setMessage({ type: "success", text: "Treatment saved successfully!" });
      if (createNext) { setSavedId(null); setFormData(structuredClone(initialForm.current)); setCurrentStep(1); }
    } catch (error) {
      setMessage({
        type: "error",
        text:
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex justify-between items-end pt-6 border-t-2 border-gray-200 mt-8">
            ...
            <button type="button" disabled={isSubmitting} onClick={e => handleSubmit(e)} className="px-6 py-3 bg-emerald-600 text-white rounded-lg font-semibold disabled:opacity-50">{isSubmitting ? 'Saving...' : 'Save'}</button>
            {currentStep < steps.length ? (
              ...
              >
                ...
                {isSubmitting ? "Submitting..." : "Save & Create Next"}
              </button>
            )}
            </div>
          </div>
